...
export default function StepConfirmation({booking,onConfirm,onBack}){
 ...
 async function confirm(){
  ...
  try{
   const result=record|| (await apiClient.post('/api/reservations',payload,{headers:{'Idempotency-Key':requestKey.current}})).reservation;
 ...
 return <section className="space-y-5"><button className="flex min-h-12 items-center gap-2 text-sm" onClick={onBack} disabled={saving||Boolean(record)}><ArrowLeft size={18}/>Retour au créneau</button><div className="rounded-3xl border border-gray-200 bg-white p-6"><p className="text-xs font-semibold uppercase tracking-widest text-primary">Votre prochain rendez-vous</p><h2 className="mt-3 text-2xl font-bold">Vérifiez, puis réservez.</h2><div className="my-6 space-y-4">{booking.services.map(service=><div key={service.id} className="border-b pb-3 font-semibold">{service.name||service.title}</div>)}<p className="flex items-center gap-3"><CalendarDays size={19}/>{booking.date?format(new Date(booking.date),'EEEE d MMMM yyyy',{locale:fr}):''}</p><p className="flex items-center gap-3"><Clock size={19}/>{booking.time}{quote&&` – ${quote.end} · ${quote.duration_min} min`}</p><p className="flex items-center gap-3"><MapPin size={19}/>{booking.salon?.name||booking.salon?.salon_name||'Au salon du professionnel'}</p><p>{persons} personne(s)</p></div>
 ...
 <button className="mt-5 min-h-12 w-full rounded-xl bg-primary p-4 font-bold text-white disabled:opacity-50" disabled={saving||loading||!quote} onClick={confirm}>{saving?'Enregistrement…':payment==='carte'?'Continuer vers le paiement':'Demander ce rendez-vous'}</button></div></section>;
}
