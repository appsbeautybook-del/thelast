...
import { creditClientFidelite } from '@/lib/fideliteClient';
...
export default function PostServiceReview({ reservation, proEmail, proName, onClose, onSubmitted }) {
  ...
  const handleSubmitReview = async () => {
    ...
    if (existingReview) {
      ...
      await entities.Avis.create({
        ...
        auteur_nom: reservation.client_name || reservation.client_email?.split("@")[0] || "Client",
      ...
      await creditClientFidelite("avis", "Avis laissé");
    }
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end bg-black/30 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-t-3xl w-full max-h-[85vh] overflow-y-auto shadow-2xl" style={{ paddingBottom: "calc(80px + env(safe-area-inset-bottom, 20px))" }} onClick={e => e.stopPropagation()}>
